    ...
    rabbitmq.subscribe(queueConfig, async (content, routingKey) => {
        try {
            ...
            if (rooms && rooms.length > 0) {
                rooms.forEach(room => {
                    try {
                        io.to(room).emit(eventName, { ...data, event: eventName, ts: Date.now() });
                    } catch (err) {
                        logger.error(`Failed to emit ${eventName} to room ${room}:`, err);
                    }
                });
                logger.info(`Cluster broadcast ${eventName} to rooms: ${rooms.join(', ')}`);
            }
            ...
            if (targetUserIds && targetUserIds.length > 0) {
                ...
                logger.info(`Cluster direct ${eventName} to users: ${targetUserIds.join(', ')}`);
            }
            ...
            try {
                ...
                if (shared.redis && typeof shared.redis.set === 'function') {
                    await shared.redis.set(
                        `delivery:${deliveryId}`,
                        JSON.stringify({ event: eventName, rooms, users: targetUserIds, ts: Date.now() }),
                        'EX',
                        86400
                    );
                }
        ...
        await rabbitmq.subscribe(
            ...
            async (content, routingKey) => {
                try {
                    ...
                    const userId = (userData && (userData._id || userData.userId)) ||
                        (content && content.data && content.data.userId) ||
                        (userData && userData.user && userData.user._id) ||
                        null;
                    if (!userId) {
                        logger.warn(`Auth event ${routingKey} missing userId in data:`, { userData, content });
                        return;
                    }

                    // Handle different auth events
                    switch (true) {
                        case eventType === 'user.registered' || routingKey.endsWith('user.registered'):
                        ...
                        case eventType === 'user.login' || routingKey.endsWith('user.login') ||
                            eventType === 'user.logged_in' || routingKey.endsWith('user.logged_in'):
                        ...
                        case eventType === 'user.logout' || routingKey.endsWith('user.logout'):
                        ...
                        case eventType === 'user.updated' || routingKey.endsWith('user.updated'):
