...
exports.getCompanyLogs = async (req, res) => {
    try {
        ...
        const { data, total } = await fetchDocs(PaymentDocument, query, page, limit);
        res.json({ success: true, data, pagination: { total, page: parseInt(page), pages: Math.ceil(total / limit) } });
    } catch (error) {
        logger.error('Error fetching company payment logs:', error);
        res.status(500).json({ success: false, message: 'Failed to fetch company payment logs' });
    }
};

exports.getShopLogs = async (req, res) => {
    try {
        ...
        logger.error('Error fetching shop payment logs:', error);
        res.status(500).json({ success: false, message: 'Failed to fetch shop payment logs' });
    }
};
