...
function start(intervalMs = 60 * 1000) {
    ...
    // Periodic processing in addition to daily cron
    setInterval(async () => {
        try { await processReminders(); } catch (e) { console.error('Reminder worker error', e && e.message ? e.message : e); }
    }, intervalMs);
    // Always schedule daily cron as well
    startDailyCron();
}

module.exports = { processReminders, startDailyCron, start };
